...
def get_args():
    ...
    parser.add_argument('-n',
                        '--num_samples',
                        help='The number of samples you would like to visualize',
                        metavar='int',
                        type=int,
                        default=0)
...
def main():
    ...
    root_list = glob.glob(os.path.join(input_dir, '*'))
    ...
    for i in range(0, len(desired_pcd_paths_list)):
        ...
        # THIS COULD MESS UP ON OTHER OPERATING SYSTEMS
        plant_name = desired_pcd_paths_list[i].split('/')[-2]

        
        gif_path_list.append(os.path.join(args.outdir, plant_name+'.gif'))
